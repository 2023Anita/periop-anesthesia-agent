...
function buildBandTrace(events = [defaultEvent]) {
  const steps = [
    ...
    ...step,
    directed_message: `${step.to_agent}\n[case_id: ${caseRecord.id}]\n[event_type: handoff]\n[handoff_from: ${step.from_agent}]\n\nTask: ${step.handoff}`,
  }));
  ...
  return {
    ...
    message_receipts: steps.map((step) => ({
      step_order: step.order,
      adapter_mode: 'local',
      sender_agent: step.from_agent,
      receiver_agent: step.to_agent,
      endpoint: 'local://band-room-transcript',
      message_id: `static-local-${step.order}`,
      delivered: true,
      detail: 'Static demo receipt mirrors the live Band Agent API message envelope.',
    })),
    audit_notes: [
      'Static public demo uses synthetic data only.',
      'The local adapter mirrors directed Band @mention handoffs without embedding live credentials in the browser demo.',
    ],
  };
}
